...
class PumpController:
    def __init__(
        ...
        - It prints information about the opened serial port and waits for the colorbot to be ready.
        ...
        if ser_port:
            ...
            # For example, to send to serial device:
            self.ser.write((self.mode + "\n").encode())
    ...
    def mqtt_task_manager(self, task, timeout_extra=0):
        ...
        try:
            msg = self.request_task(task, req_id, result_q, timeout_extra)
            return msg
        finally:
            self.running_task = False
    ...
    def send_to_colorbot(self, send_str):
        ...
        Parameters:
        - send_str (str): The string message to be sent to the colorbot.

        Returns:
        - None

        Notes:
        - The message is encoded as UTF-8 before being sent to the colorbot.
    ...
    def recv_from_colorbot(self, timeout=10.0):
        ...
        Parameters:
        - timeout (float): Maximum time (in seconds) to wait for a response from the colorbot.

        Returns:
        - str: The received data from the colorbot.

        Raises:
        - TimeoutError: If the colorbot does not respond within the specified timeout.

        Notes:
        - This method reads data from the colorbot until it encounters the end marker (ASCII 62).
    ...
    def wait_for_colorbot(self):
        ...
        Notes:
        - This method continuously checks for the "colorbot is ready" message from the colorbot.
        - It uses the `recv_from_colorbot` method to receive data and prints the received message.
        - The method waits until there is data available in the serial buffer (`in_waiting` > 0).
        - The process continues until the colorbot signals readiness, as indicated by the message.
        """
    ...
    def run_test(self, test_str):
        ...
        Parameters:
        - test_str (str): The test message to be sent to the colorbot.
        ...
        - It sends the test message to the colorbot using the `send_to_colorbot` method.
        ...
        if waiting_for_reply:
            ...
            data_received = self.recv_from_colorbot()
